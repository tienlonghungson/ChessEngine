...
public interface ActiveBoard {
    ...
    /**
     * get ZobristKey of current state of this board
     * @return ZobristKey is an 64-bit integer.
     */
    long getZobristKey(boolean isWhite);

    /**
     * update Zobrist key when a move is executed
     * @param move the move caused the update
     * @return updated zobrist key
     */
    long updateZobristKey(Move move);

    /**
     * check if the current status allows null-move
     * @param isWhite which side???
     * @return {@code true} if null-move is allowed
     */
    boolean hasNullMove(boolean isWhite);
}
